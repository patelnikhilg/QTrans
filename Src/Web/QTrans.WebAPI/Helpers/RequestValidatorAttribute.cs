...
namespace QTrans.WebAPI.Helpers
{
    public class RequestValidatorAttribute : ActionFilterAttribute
    {
            ...
            if (!skipAuthorization)
            {
                if (actionContext.Request.Headers.Contains("AuthorizationToken"))
                {
                    ...
                    if (value != null)
                    {
                        try
                        {
                            if (UsersSession.userTokenDic.ContainsKey(value))
                            {
                                _isValidUser = true;
                            }
                            else
        ...
        public bool validateAPIKey(string _apiKey)
        {
            bool _isApiKey = false;
            ...
            string apiKey = ConfigurationManager.AppSettings["mobileApiKey"].ToString();
